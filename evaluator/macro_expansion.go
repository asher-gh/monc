...
func DefineMacros(prog *ast.Program, env *object.Environment) {
	...
	for i := len(definitions) - 1; i >= 0; i -= 1 {
		...
		prog.Statements = append(prog.Statements[:definitionIndex], prog.Statements[definitionIndex+1:]...)
	}
}

func ExpandMacros(prog *ast.Program, env *object.Environment) ast.Node {
	return ast.Modify(prog, func(node ast.Node) ast.Node {
		...
		if !ok {
			panic("we oly support returning AST-nodes from macros")
		}
